...
1. hero — skill: hero — variant: typographic over showpiece backdrop — density: H1 ≤7 words + sub ≤22 + 1 scroll-cue CTA + mono meta line (`1999 — 2026 // 3 engines // 9 maps`) — width: full-bleed — rhythm: open, huge air — job: state the thesis — SIGNATURE: THE COMPILE — persistent R3F diorama pinned as the scroll spine of sections 1–6, compiling wireframe→blockout→textured→lit as the acts pass (reduced-motion/no-WebGL fallback: per-era static SVG posters + sr-only narrative).
