...
Exact version numbers live in **stack/versions.json** — the single authority, refreshed with `node scripts/verify-stack.mjs` — and are deliberately NOT restated here, so this file can never contradict the manifest again. What this line keeps is the qualitative flags the digits can't carry: drizzle-orm has a stable line and an `@rc` line the official docs install — `database` records the choice, never mixes them · next-auth is REJECTED (v5 never left beta); better-auth is the auth stack · Content Collections installs as `@content-collections/core` + `@content-collections/next` (the bare npm name is the project's installer CLI, not the library) · animejs and the whole three.js/R3F set are DIRECTION-gated, never in the base install · three is pinned exactly with `@types/three` locked to it and `@next/mdx` locked to next (verify-stack enforces both locks) · postprocessing's peer range caps three (see the R3F section).
...
- **v13 major, adopted after a focused pass:** the documented breaking change is React prop filtering for CSS-in-JS libraries (styled-components/emotion) — N/A here, ultraweb styles exclusively with Tailwind tokens. The core surface above re-verified green on 13.1.1 via install + production build + typecheck AND a Playwright runtime smoke: entrance animation completes, AnimatePresence exit removes the node, useScroll→useSpring updates the transform on scroll, reduced-motion renders the static final state, and a separate strict `domMax` provider mounts with working drag + layout animation. The +15/+25 kb feature-bundle deltas were measured on 12.x and are **UNVERIFIED for v13** until remeasured — cite them as 12.x history, never as v13 fact.

## three.js / React Three Fiber — the facts (DIRECTION-gated)

MIT — except `postprocessing`, which is Zlib (both permissive; no copyleft exposure). `motion` remains THE animation library and `animejs` the one specialist *animation* engine; three.js/R3F is a **renderer** and the heaviest dependency decision in this harness, installed in Phase 9 only when design/DIRECTION.md commissions 3D BY NAME. `showpiece` owns the single-set-piece gate; `ultraweb:set-design` owns the site-scale gate (persistent canvas, scroll-driven camera, one material world) and additionally requires the route scope, the byte budget and the static edition in writing. `motion-language` owns the engine-ownership table. Everything below was verified against the installed tarballs (source reads, export enumeration, DOM-less Node execution), a real Next 16 production build (on the then-current 16.2 line at verification time), and esbuild+gzip measurement.
...
- **Ecosystem note (observed 2026-07-29, manifest since refreshed):** `typescript` `latest` is on the 7.x line, which Next refuses without `experimental.useTypeScriptCli` — create-next-app still scaffolds a compatible 5.x; leave it alone. Current digits live in stack/versions.json.
...
- **Content:** Contentlayer is DEAD — never recommend it. Options: plain `@next/mdx` (requires root `mdx-components.tsx` exporting `useMDXComponents` — forgetting it is the #1 setup error; `pageExtensions` must include md/mdx), **content-collections** (install `@content-collections/core` + `@content-collections/next` — the bare `content-collections` npm name is the project's own installer CLI (versions stop at 0.2.x) — not the library), or **velite** for typed content collections.
