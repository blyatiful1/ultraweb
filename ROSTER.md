...
**Progressive disclosure:** a skill's SKILL.md is its decision core; worked examples, compose maps, and catalogs above ~12KB live in `skills/<name>/references/` and load only when a build genuinely needs them (`example.md`, `composes.md`; award-canon also carries `ARCHETYPE-MAP.md`, `INVARIANTS.md`, `CANON.md`). New skills follow the same split. Since v1.9.0 the split is complete: every skill with a worked example or compose map keeps only the two-line stub in SKILL.md (`scripts/split-references.mjs` does the move; lint refuses an inline body), the root skill's own worked example lives in `references/example.md`, and `direction`'s twelve-archetype catalog in `skills/direction/references/CATALOG.md`. The six measurement gates (Tier 7 minus `gate-visual`) are loaded by the `gate-runner` subagent, never by the Lead. `scripts/context-budget.mjs` reports what a standard build loads per phase and fails CI over its ceiling. Corpus invariants are enforced by `scripts/lint-skills.mjs` (run by CI on every push); stack pins live in `stack/versions.json`, refreshed by `scripts/verify-stack.mjs`.
...
Six of these run inside the `gate-runner` subagent (Sonnet 5), strictly sequentially against the production server of record the Lead starts once — code → responsive → antislop → content → accessibility → performance. Every checklist item is tagged MEASURED / OBSERVED / JUDGMENT: the runner asserts the first two and returns the third with evidence for the Lead to rule on; its browser measurements come from the shared library in `scripts/measure/`. `gate-visual` is the Lead-run judgment loop.
...
- **gate-visual** — the self-critique loop: screenshot every page, judge against DIRECTION.md + taste with a scored rubric (hierarchy, typography, spacing, color, distinctiveness, craft), fix worst issue, repeat — one round at sketch, ≥2 at standard, up to five at flagship. *(running app → QA.md)*
...
- **pixel-qa** (`model: sonnet` — Sonnet 5, mechanical sweeps) — drives Playwright MCP: navigates, screenshots at breakpoints, reports console errors and layout breaks. Used by gate-visual (and retrofit's audit sweep); the responsive sweep itself runs inside gate-runner.
...
- **gate-runner** (`model: sonnet` — Sonnet 5, measurement work) — executes one measurement gate end to end against the production server of record: loads the gate skill, runs every MEASURED/OBSERVED step from the measurement library, collects JUDGMENT evidence, appends the QA.md entry, writes qa/<gate>.log, returns ≤400 tokens. Used by Phase 11 (one dispatch per gate, in sequence) and by iterate re-gates.
