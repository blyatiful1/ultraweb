...
7. **Production smoke test — the core of this skill.** Kill any dev server. `npm run build` → exit 0. `npm run start` → fetch every route in design/SITEMAP.md on localhost:3000 expecting 200, plus `/robots.txt` and `/sitemap.xml` — and `/studio` expecting **404** (the construction-site route must be dead in production; a 200 here is a ship blocker) → zero errors in server output → kill it. Dev green ≠ prod green: the production server surfaces missing runtime env and real caching behavior (`fetch` is not cached by default; `'use cache'` boundaries now actually run).
