...
- **vs `physics`.** If the thing that moves is a DOM node, it is physics. If it is an `Object3D`, it is this skill. Pointer→scene raycasting and camera damping are scene input; every cursor follower, magnetic hover and DOM spring stays on motion at `domMax`, and no second drag system is installed. `OrbitControls`/`CameraControls` are not installed on a site-scale scene — a free camera fights an authored journey, and a camera the user can lose is not a site.
...
import { useScroll } from "motion/react";          // real page scroll, not drei's
...
One asset closes the section: **the per-route poster frame is also that route's `opengraph-image`** — one build-time render of the GLB at each window's start, shipped as AVIF, reused as the static tier, the `reduce` tier, the no-JS tier and the social card. Four deliverables, one asset.
