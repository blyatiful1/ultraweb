...
- **The GL chunk is never first-load.** `next/dynamic({ssr:false})` inside a client leaf, requested after gate-performance's cold-load cutoff (the post-`load` network-quiet window — which is always after the LCP entry, so LCP stays protected too). Cold-load JS stays ≤140 kB per route; the GL chunk is measured separately against its own written budget. The LCP element is server-rendered text or image on every route, always.
...
7. **Verify empirically, per route.** Performance recording ≥5s of scroll plus one navigation on *every* route in the scope; 4× CPU throttle, recording which rung of the adaptive ladder it settles on; reduced-motion emulation on every route; JS off; keyboard-only pass; `npm run build`, then measure the GL chunk's gzip from the network log (or `npx next experimental-analyze --output` — the build table prints no sizes in Next 16) and hand it plus the DIRECTION.md citation to `gate-performance`.
...
The carve-out is honest, not a loophole: the GL chunk is genuinely absent from the cold-load figure only when its request starts **after gate-performance's cold-load cutoff** (the 500ms post-`load` network-quiet window — one cutoff for budget and carve-out alike, so the exclusion and the figure of record can never disagree), and `gate-performance` verifies that **in the network waterfall** — Next 16's build output prints no per-route sizes, so the waterfall is the only witness. A renderer inside cold-load JS is a mounting defect, not a budget negotiation.
...
4. Cold-load JS ≤140 kB per route measured from the network log, with the GL chunk excluded only when its request starts after the cold-load cutoff; the GL chunk's measured gzip recorded against its DIRECTION.md budget; total transfer per route under budget and under 3 MB.
