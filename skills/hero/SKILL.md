...
- An SVG signature move is never the LCP element and never mounts hidden. The server component paints the finished static SVG into the initial HTML — real geometry, full stroke — and the client leaf (CSS keyframes, or a DIRECTION-commissioned `ultraweb:animejs` timeline) rewinds and replays it after hydration. A path shipped at full `stroke-dashoffset` waiting for JS is invisible content: it fails a JS-off load, it fails reduced motion, and if it were the LCP candidate the metric would slide to whenever the draw finishes.
...
- One entrance choreography maximum: headline → subline → CTA, 400-700ms total, 40-80ms stagger, then STOP. Nothing in the hero loops forever except a `ultraweb:showpiece` element that earned it, a commissioned `ultraweb:set-design` scene (where the loop is the medium, per `ultraweb:motion-language`), the scroll cue (Supporting cast), or a DIRECTION-sanctioned living idle per `ultraweb:motion-language` (≤2 elements, transform-only, behind reduced-motion).
- A heavy showpiece hero earns a designed loading moment, never a spinner (`award-canon`: The Loader is the Overture): its final frame IS the hero's first frame for a seamless handoff, it stays skippable, and it cuts instantly under reduced-motion. `page-transitions`/`showpiece`/`set-design` own the mechanism; the hero owns the frame handed to it — and on a `set-design` build that frame is the poster of the `/` window, already baked as the route's static edition.
...
- ultraweb:set-design — when the commissioned world persists across routes, the first viewport still renders headline and CTA from server HTML in front of the scene, never behind it.
