...
- A JS scroll listener or `useScroll` driving an effect that's a pure function of scroll position — a native `animation-timeline` does it on the compositor; gate-performance flags these (exception: the DIRECTION-commissioned animejs scrubbed-SVG moment — `animation-timeline` cannot drive multi-path timeline choreography).
