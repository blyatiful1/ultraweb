...
- ultraweb:set-design — authors the per-route poster frame that is simultaneously the static tier, the reduced-motion tier, the no-JS tier and that route's `opengraph-image`; on a persistent-scene build the poster is a deliverable on every route, not one fallback
