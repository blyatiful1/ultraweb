...
3. **Dead copy & microcopy** — zero hits on the taste absolutes and copywriting's expanded banned list; every testimonial/review/press attribution traces to design/BRIEF.md's proof inventory (untraceable proof is fabricated, a defect regardless of how well it reads), and zero `UNVERIFIED-PROOF` tags unless BRIEF.md marks the build demo/staging; and the same rigor on controls: every button/link label is verb+object, never a bare "Submit"/"OK"/"Learn more"/"Click here"; every destructive confirm names its consequence, not "Are you sure?"; every tooltip adds information beyond its trigger or is deleted; no bare "Something went wrong" without an adjacent recovery; ≤1 "Learn more" per page.
...
**Proof trace.** `rg -n "UNVERIFIED-PROOF" . -g '!design/**'` (root-wide, gitignore-aware, test/fixture trees excluded) — zero hits on a production build; a hit passes only when design/BRIEF.md's `Deployment mode:` line reads `staging` or `demo` AND the rendered quote carries a visible "Sample" label (screenshot it as evidence). Where the build uses social-proof's structured proof store, also assert every record's `status` is `"verified"` on a production build — the store is the provable surface. Then collect every `<figcaption>`/attribution in testimonial and review components and trace each name+company to BRIEF.md's proof inventory. A quote attributed to someone BRIEF.md never supplied is fabricated social proof — a launch-blocking defect, not a copy nit; the fix is social-proof's no-proof ladder (cut the section or replace with honest credibility), never a better-sounding fake.
...
- Passing a testimonial because it reads well — attribution that traces to no BRIEF.md proof inventory is fabricated, and polish makes that worse, not better
