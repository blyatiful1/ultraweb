...
**Proof trace.** `rg -n "UNVERIFIED-PROOF" app components content emails` — zero hits on a production build; a hit passes only when design/BRIEF.md explicitly marks the build demo/staging AND the rendered quote carries a visible "Sample" label (screenshot it as evidence). Then collect every `<figcaption>`/attribution in testimonial and review components and trace each name+company to BRIEF.md's proof inventory. A quote attributed to someone BRIEF.md never supplied is fabricated social proof — a launch-blocking defect, not a copy nit; the fix is social-proof's no-proof ladder (cut the section or replace with honest credibility), never a better-sounding fake.
