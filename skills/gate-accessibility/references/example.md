...
Step 8 surfaced a quieter defect: under the text-spacing override, the `/grants` cards — grant title `line-clamp-2` inside a fixed `h-56` — clipped the longest programme name mid-word. ultraweb:cards traded the hard height for `min-h-56` + `flex-col`; re-shot at 375/768/1440 under the override, every card grew clean. Step 9 didn't fire — design/BRIEF.md sets market=UK, so the check logged N/A. A DACH brief runs the scoping gate instead of assuming a duty: Ledger & Lane, a two-partner German firm whose site concludes no consumer contracts online, records OUT (brochure scope — and likely the microenterprise exemption on top); only an in-scope build ships the § 14 information, with its claimed level matched to this run's residuals.
